...
class FarmActivity{
    ...
    function updateFarmActivity() {
        ...
        // Sanitize & type-cast
        $this->activity_name       = strip_tags($this->activity_name);
        $this->farm_activity_type  = strip_tags($this->farm_activity_type);
        $this->additional_info     = strip_tags($this->additional_info);
        $this->activity_date       = strip_tags($this->activity_date);

        $this->activity_cost       = floatval($this->activity_cost);
        // $this->farm_resource_id    = intval($this->farm_resource_id);
        $this->id                  = intval($this->id);

        // Bind params
        $stmt->bindParam(":activity_name", $this->activity_name, PDO::PARAM_STR);
        $stmt->bindParam(":farm_activity_type", $this->farm_activity_type, PDO::PARAM_STR);
        $stmt->bindParam(":activity_cost", $this->activity_cost);
        $stmt->bindParam(":activity_date", $this->activity_date, PDO::PARAM_STR);
        $stmt->bindParam(":additional_info", $this->additional_info, PDO::PARAM_STR);
