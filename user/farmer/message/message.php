...
$customer_id = isset($_GET['cus_id']) ? intval($_GET['cus_id']) : 0;
...
$consumer_info = new User($db);
$consumer_info->id = $customer_id;
$consumer_info->getConsumerInfo();
$image = $consumer_info->profile_pic;
$default_image = "{$base_url}user/uploads/logo.png";
$consumer_profile_path = "{$base_url}user/uploads/profile_picures/consumer/{$customer_id}/$image";
// Check if file exists on server
if (!empty($image) && file_exists($_SERVER['DOCUMENT_ROOT'] . "/user/uploads/profile_picures/consumer/{$customer_id}/$image")) {
    ...
    <div class="col-md-8">
      <div class="card h-100 shadow-sm p-3" id="chatContent">
        ...
        <div class="avatar bg-purple text-white rounded-circle d-flex align-items-center justify-content-center me-2" style="width:50px; height:50px; overflow:hidden;">
          ...
          <strong><?= $consumer_info->firstname ?></strong>
        </div>
...
let receiver_id  = <?= intval($customer_id); ?>; //farmer id
let customer_id = <?= intval($customer_id); ?>;
    ...
    $.get("getConversation.php", function(data) {
        ...
        convs.forEach(c => {
            const li = $('<li>')
                ...
                .attr('data-sender-id', c.sender_id)
...
// -----------------------------------------------------
  $("#conversationList").on("click", "li", function () {
      ...
      const sender_id = $(this).data("sender-id");

      currentConversation = convId;

      const newUrl = window.location.pathname + "?cus_id=" + sender_id + "&cid=" + convId;
