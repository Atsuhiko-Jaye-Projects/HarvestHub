...
$pre_order_crop_stmt = $crop->readAllPreOrderCrop($from_record_num, $records_per_page);
$pre_order_crop_num = $pre_order_crop_stmt->rowCount();
$pre_order_total_rows = $crop->countAll();
    ...
    <table class="table table-hover table-bordered align-middle mb-0 text-center" style="table-layout: fixed; width: 100%;">
        <thead class="table-success text-uppercase">
            <tr>
                ...
                <th style="width: 10%;"><i class="bi bi-clipboard-data me-1"></i> Pre-Ordered (KG)</th>
                <th style="width: 10%;"><i class="bi bi-calendar-plus me-1"></i> Date Planted</th>
                <th style="width: 10%;"><i class="bi bi-calendar-check me-1"></i> Harvest Date Est.</th>
        ...
        <tbody id="crop_table" class="table-group-divider text-center">
            <?php
            if ($pre_order_crop_num > 0) {
                while ($row = $pre_order_crop_stmt->fetch(PDO::FETCH_ASSOC)) {
                    extract($row);
                    ...
                    echo "<tr>";
                        ...
                        echo "<td class='text-truncate'>{$row['reserve_kg']} KG</td>";
